...
const Upload = ({ modes, onSuggestions, onUploadStatus }) => {
  const Upload = () => {
    ...
    const handleUpload = async () => {
      ...
      const ApiService = new ApiService("http://localhost:8504");
      const apiResponse = await ApiService.upload(formData);
      ...
      if (uploadResponse.ok) {
        ...
        if (ocrResponse.ok) {
          ...
          await analyzeText(result.uuid, ocrResult.text); // Pass the UUID and text for analysis
        }
      } else {
    ...
    const analyzeText = async (uuid, extractedText) => {
      const service = new ApiService("http://localhost:8504");
      const instructions = {
